...
namespace INFINITY
{
...

    {
...

        {
            ...
            MeshRenderers = gameObject.GetComponent<MeshRenderer>();
...

        {
            ...
            if (Time.time - lastCombinedTime > interval && INFINITY.settings_.afterimageOn)
            ...
            meshRenderer.material = MeshRenderers.material;



            // 初始化残影对象，设置位置、旋转、生命周期

            InitFadeInObj(go, MeshRenderers.transform.localScale, MeshRenderers.transform.position, MeshRenderers.transform.rotation, lifeCycle);
            }
        }
        ...
        private void InitFadeInObj(GameObject go,Vector3 localScale , Vector3 position, Quaternion rotation, float lifeCycle)

        {
            ...
            go.transform.position = position + new Vector3(0, 0, 0.001f);
